...
namespace WindowsFormsApplication1
{
    public partial class Difficult : Form
    {
        ...
        const int MAX_WIDTH = 15;   // 列数
        const int MAX_HEIGHT = 15;  // 行数
        ...
        Button[,] button = new Button[MAX_WIDTH, MAX_WIDTH];
        bool chooseState = true;//取值是true的时候，选中下面判断点亮的按钮，取值是false的时候，选中下面判断是错的按钮，初值为true
        bool isMusic = true;   //判断音乐是否打开的布尔值

        int bGame;          // 总共需要点亮的格子，判断游戏是否结束
        int life;      //总共可以错误的次数，判断游戏是否结束

        public Difficult()  //初始化困难版本的游戏界面
        {
        ...
        private void initButton()
        {
            ...
            for (i = 0; i < MAX_WIDTH; i++)
            {
                for (j = 0; j < MAX_HEIGHT; j++)
                {
                    ...
                    button[i, j].BackColor = Color.Gray;//将每个按钮的背影颜色置为灰色
                    //把按钮加入窗口
                    this.Controls.Add(button[i, j]);
                    //把下一个按钮的位置右移40像素
                    Left += 24;
                    //当Left大于200的时候，就要换行，则Left重置为29，Top加40
                    if (Left >= 380)
                    {
                        Left = 39;
                        Top += 24;
        ...
        public void initRightAnswer()
        {
             ...
             * labelOne到labelFive分别代表了界面上的第一行提示到第五行提示
             * 通过用toString方法把数组里面存储的数字转换成为字符，每转换一个数字就要添加一个空格，和后一个数字分隔开
             */
            for (j = 0; j < MAX_HEIGHT; j++)
            {
                if (count[0, j] != 0)
                    label1.Text += count[0, j].ToString() + ' ';

                if (count[1, j] != 0)
                    label2.Text += count[1, j].ToString() + ' ';

                if (count[2, j] != 0)
                    label3.Text += count[2, j].ToString() + ' ';

                if (count[3, j] != 0)
                    label4.Text += count[3, j].ToString() + ' ';

                if (count[4, j] != 0)
                    label5.Text += count[4, j].ToString() + ' ';

                if (count[5, j] != 0)
                    label6.Text += count[5, j].ToString() + ' ';

                if (count[6, j] != 0)
                    label7.Text += count[6, j].ToString() + ' ';

                if (count[7, j] != 0)
                    label8.Text += count[7, j].ToString() + ' ';

                if (count[8, j] != 0)
                    label9.Text += count[8, j].ToString() + ' ';

                if (count[9, j] != 0)
                    label10.Text += count[9, j].ToString() + ' ';

                if (count[10, j] != 0)
                    label11.Text += count[10, j].ToString() + ' ';

                if (count[11, j] != 0)
                    label12.Text += count[11, j].ToString() + ' ';

                if (count[12, j] != 0)
                    label13.Text += count[12, j].ToString() + ' ';

                if (count[13, j] != 0)
                    label14.Text += count[13, j].ToString() + ' ';

                if (count[14, j] != 0)
                    label15.Text += count[14, j].ToString() + ' ';
            ...
            {
                if (count1[0, j] != 0)
                    label16.Text += count1[0, j].ToString() + '\n';

                if (count1[1, j] != 0)
                    label17.Text += count1[1, j].ToString() + '\n';

                if (count1[2, j] != 0)
                    label18.Text += count1[2, j].ToString() + '\n';

                if (count1[3, j] != 0)
                    label19.Text += count1[3, j].ToString() + '\n';

                if (count1[4, j] != 0)
                    label20.Text += count1[4, j].ToString() + '\n';

                if (count1[5, j] != 0)
                    label21.Text += count1[5, j].ToString() + '\n';

                if (count1[6, j] != 0)
                    label22.Text += count1[6, j].ToString() + '\n';

                if (count1[7, j] != 0)
                    label23.Text += count1[7, j].ToString() + '\n';

                if (count1[8, j] != 0)
                    label24.Text += count1[8, j].ToString() + '\n';

                if (count1[9, j] != 0)
                    label25.Text += count1[9, j].ToString() + '\n';

                if (count1[10, j] != 0)
                    label26.Text += count1[10, j].ToString() + '\n';

                if (count1[11, j] != 0)
                    label27.Text += count1[11, j].ToString() + '\n';

                if (count1[12, j] != 0)
                    label28.Text += count1[12, j].ToString() + '\n';

                if (count1[13, j] != 0)
                    label29.Text += count1[13, j].ToString() + '\n';

                if (count1[14, j] != 0)
                    label30.Text += count1[14, j].ToString() + '\n';
            }
        }
        ...
        private void changeButtonImage(int flag, int buttonX, int buttonY)
        {
            ...
            {
                button[buttonX, buttonY].Image = WindowsFormsApplication1.Properties.Resources.radioFalse10;    //设置按钮的图片为名字为radio5的红色叉
            }
            else
            {
                button[buttonX, buttonY].Image = WindowsFormsApplication1.Properties.Resources.judgeFalse10;  //设置按钮的图片为名字为judgeFalse5的蓝色叉
            }
            this.Refresh();
        }

        /*
         * 自己定义的click，通过把这个click事件添加到button数组里面，让button被点击之后都调用这个click事件
         */
        ...
        //当这个窗口被关闭，关掉音乐
        private void Difficult_FormClosing(object sender, FormClosingEventArgs e)
        {
            music.Stop();
        }
