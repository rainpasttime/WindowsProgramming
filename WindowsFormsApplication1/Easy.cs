...
namespace WindowsFormsApplication1
{
    public partial class Easy : Form
    {
        ...
        const int MAX_WIDTH = 5;   // 列数
        const int MAX_HEIGHT = 5;  // 行数
        ...
        public void initRightAnswer()
        {
            ...
            {
                if (count1[0, j] != 0)
                    labelSix.Text += count1[0, j].ToString() + '\n';

                if (count1[1, j] != 0)
                    labelSeven.Text += count1[1, j].ToString() + '\n';

                if (count1[2, j] != 0)
                    labelEight.Text += count1[2, j].ToString() + '\n';

                if (count1[3, j] != 0)
                    labelNine.Text += count1[3, j].ToString() + '\n';

                if (count1[4, j] != 0)
                    labelTen.Text += count1[4, j].ToString() + '\n';
            }
        }
